...
class Characteristic:
    ...
    def read2(self):
        return self.peripheral.readCharacteristic2(self.valHandle)
...
#
    ...
    #        self._helper.stdout.flush()
    #        #print ('original rv:%s' % rv)
    #        DBG("Got:", repr(rv))
    #        if rv.startswith('#'):
    #            dataPlus = True
    #            continue
...
#
    ...
    #            if dataPlus == True:
    #                beforeData['d'][0] = beforeData['d'][0]+resp['d'][0]
    #                #print ('      Data = %s' % beforeData)
    #                return beforeData
    #            beforeData = resp
    #            #print ('first Data = %s' % beforeData)
#
    #            dataPlus = True
    #            continue
    #            #return resp
    ...
    def _getResp(self, wantType):
        willRdNtfyReturnData = ''
        _getRespCount = 0
        while True:
            _getRespCount = _getRespCount + 1
            #print ('_getRespCount : %d' % _getRespCount)
            if self._helper.poll() is not None:
                raise BTLEException(BTLEException.INTERNAL_ERROR, "Helper exited")
            ...
            if respType == wantType:
                if respType == 'rd':
                    resp['d'][0] = willRdNtfyReturnData
                #print ('respType return :%s' % resp)
                return resp
            elif respType == 'stat' and resp['state'][0] == 'disc':
                self._stopHelper()
                raise BTLEException(BTLEException.DISCONNECTED, "Device disconnected")
                ...
                DBG("Ignoring notification")
                #print ('ntfy : %s' % resp['d'][0])
                willRdNtfyReturnData = willRdNtfyReturnData + resp['d'][0]
                #print ('willRdNtfyReturnData : %s' % willRdNtfyReturnData)
                DBG(resp['d'][0])
                continue
                #return resp
            else:
    ...
    def readCharacteristic2(self, handle):
        self._writeCmd("rd %X\n" % handle)
        resp = self._getResp2('rd')
        return resp['d'][0]
